<x-layout>
    <div class="w-full space-y-6">
        ...
        <div class="relative w-full flex justify-between gap-6">
                ...
                <div class="bg-white rounded-lg space-y-2 pb-6 shadow">
                    ...
                    <div class="py-2 px-4 space-y-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-base">{{$campaign->title}}</p>
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                ...
                                <p class="text-[#155DFC] text-lg">{{$campaign->fundraising_progress}}%</p>
                            </div>
                            <div class="h-3 w-full bg-[#F3F4F6] rounded-full">
                                <div 
                                    class="h-3 w-full bg-clip-padding bg-gradient-to-r from-blue-800 to-violet-500 rounded-full"
                        ...
                        <div class="w-full flex items-center justify-between gap-4">
                            <div class="space-y-1 w-1/2 p-5 bg-clip-padding bg-gradient-to-r from-[#EFF6FF] to-[#DBEAFE] rounded-lg">
                                ...
                                <p class="text-[#155DFC] text-2xl font-bold">Rp {{ number_format($campaign->current_amount, 0, ',', '.') }}</p>
                            </div>
                            <div class="space-y-1 w-1/2 p-5 bg-clip-padding bg-gradient-to-r from-[#F9FAFB] to-[#F3F4F6] rounded-lg">
                                ...
                                <p class="text-[#364153] text-2xl font-bold">Rp {{ number_format($campaign->target_amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                        <div class="w-full border-t pt-6 space-y-2">
                            <p class="text-[#0A0A0A] text-base">Description</p>
                            <p class="text-[#364153] text-sm">{{$campaign->description}}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        <div class="p-4 bg-[#F9FAFB] flex items-start gap-3 rounded-lg">
                            ...
                            <div class="space-y-1">
                                ...
                                <p class="text-[#101828] text-base font-semibold">{{$campaign->Bank->bank_name}}</p>
                            </div>
                        </div>
                        <div class="p-4 bg-[#F9FAFB] flex items-start gap-3 rounded-lg">
                            ...
                            <div class="space-y-1">
                                ...
                                <p class="text-[#101828] text-base font-semibold">{{$campaign->Bank->account_number}}</p>
                            </div>
                        </div>
                        <div class="p-4 bg-[#F9FAFB] flex items-start gap-3 rounded-lg">
                            <i class="bi bi-credit-card text-blue-500"></i>
                            <div class="space-y-1">
                                ...
                                <p class="text-[#101828] text-base font-semibold">{{$campaign->Bank->account_holder}}</p>
                            </div>
                        </div>
                    </div>
            ...
            <form
                action="{{ route('donations.store', $campaign->id) }}"
                method="POST"
                class="sticky top-0 w-1/3 bg-white rounded-lg p-6 space-y-6 h-fit shadow"
            >
                @csrf
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <div class="w-full flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-1 outline-1 -outline-offset-1 outline-gray-300">
                            ...
                            <input id="donatur_name" type="text" name="donatur_name" placeholder="Anonymous" class="block min-w-0 grow bg-white py-1.5 pr-3 pl-1 text-base placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
                        </div>
